...
Get-QADComputer -OSName "Windows*Server*" | select name |out-file c:\temp\computernames.txt
Start-Sleep -s 30
...
.EXAMPLE
Get-RunAsAccount -ComputerName (Get-Content c:\temp\computernames.txt)
.EXAMPLE
...
Start-Sleep -s 5
Get-RunAsAccount -ComputerName (Get-Content c:\temp\computernames.txt)|export-csv c:\temp\services.txt -notypeinformation