...
#
#    b) Execute serverinfo.ps1 and remember to cross check the file locations within otherwise it runs the risk of not working properly
#       powershell c:\scripts\getinfo-servers.ps1
#
# 6) Review the results:
#    notepad c:\temp\serverdata.csv
#----------------------------------------------------------------------------------------------------------------------
# NOTES: 
...
Function GetInfo {
      ...
      [string[]]$computername, 
      [string]$logfile = 'c:\temp\unreachable.txt',
      [string]$outfile = 'c:\temp\computerinfo.csv'
...
PROCESS {
      ...
      Foreach ($computer in $computername) {
            ...
            if ($continue) {
                  ...
                  [int]$cpudata = 0
                  (Get-WmiObject -class win32_processor -ComputerName $computer -Property numberofcores | select NumberOfCores).NumberOfCores | % {$cpudata += [int]$_ }


                  $Drives = $null
                  (Get-WmiObject -Class win32_logicaldisk -cn $Computer | select DeviceID, DriveType| where DriveType -eq 3).DeviceID | % {
                  Get-WmiObject -Class win32_logicaldisk -cn $Computer | where deviceID -like $_ | % {$Drive =  "$([math]::round($_.Size /1Gb,2))"}
                  $Drives += "$_$Drive,"
    }
                  ...
                  if ($phyv -like "*-*" -or $phyv -like "*VM*" -or $phyv -like "*vm*") { $res = "Virtual" } 
...
# - For "report-ONLY," you can comment out the "$obj" (11 lines) below
                   ...
                   $obj | Add-Member –membertype NoteProperty –name ComputerName –value ($computer) –PassThru |
                          ...
                          Add-Member –membertype NoteProperty –name Space -value ($Drives.Substring(0,($Drives.length - 1))) -PassThru |
                          ...
                          Add-Member –membertype NoteProperty –name Cores –value ($cpudata)
