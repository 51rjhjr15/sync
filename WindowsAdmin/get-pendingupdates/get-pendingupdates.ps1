...
End { 
    ...
    $obj | Add-Member –membertype NoteProperty –name ComputerName –value ($computer) –PassThru |
           Add-Member –membertype NoteProperty –name patches –value ($count)
...
$patches = Get-Content C:\temp\servers.txt
ForEach ($patches in Get-Content C:\temp\servers.txt) {

    Get-PendingUpdates -Computer $patches | Export-CSV -Path C:\temp\patches.csv -Append -NoTypeInformation
    }
